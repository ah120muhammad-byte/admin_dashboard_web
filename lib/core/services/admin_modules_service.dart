...
class AcademicLevelOption {
  ...
  factory AcademicLevelOption.fromMap(Map<String, dynamic> map) {
...
class AdminModulesService {
  ...
  AdminModulesService({SupabaseClient? supabase})
  ...
  Future<List<AdminModule>> getModules({
    ...
    var query = _supabase.from('modules').select(
          'id, academic_level_id, name, description, image_url, '
          'display_order, is_active, created_at, updated_at',
        );

    if (academicLevelId != null && academicLevelId.isNotEmpty) {
      query = query.eq('academic_level_id', academicLevelId);
    }
    ...
    return (response as List)
        .map((item) => AdminModule.fromMap(Map<String, dynamic>.from(item)))
        .toList();
  }

  Future<List<AcademicLevelOption>> getAcademicLevels() async {
    ...
    return (response as List)
        .map((item) => AcademicLevelOption.fromMap(Map<String, dynamic>.from(item)))
        .toList();
  }
  ...
  Future<void> updateModule({
    ...
    await _supabase.from('modules').update({
      'academic_level_id': academicLevelId,
      'name': name,
      'description': description,
      'image_url': imageUrl,
